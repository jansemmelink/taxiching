...
func main() {
	...
	adminUser, _ := users.New("admin", "admin")
	...
	if err != nil {
		panic(log.Wrapf(err, "Failed to create unknown user wallet"))
	}

	adminSession, err := sessions.New("admin", "admin")
	if err != nil {
		panic(log.Wrapf(err, "failed to create admin session"))
	}

	_ /*user1*/, wallet1 := newUser("one", "aa11")
	_ /*user2*/, wallet2 := newUser("two", "bb22")
	...
	sessionUser1, err := sessions.New("one", "aa11")
	if err != nil {
...
func newUser(n, p string) (users.IUser, wallets.IWallet) {
